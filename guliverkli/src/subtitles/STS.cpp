...
static bool OpenMicroDVD(CTextFile* file, CSimpleTextSubtitle& ret, int CharSet)
{
	...
	while(file->ReadString(buff))
	{
		...
		if(c == 2)
		{
			if(fCheck2 && ret.GetSize())
			{
				STSEntry& stse = ret[ret.GetSize()-1];
...
void CSimpleTextSubtitle::Append(CSimpleTextSubtitle& sts, int timeoff)
{
	if(timeoff < 0) 
	{
		timeoff = GetSize() > 0 ? GetAt(GetSize()-1).end : 0;
	}
	...
	for(int i = 0, j = sts.GetSize(); i < j; i++)
	{
		STSEntry stse = sts.GetAt(i);
		stse.start += timeoff;
		stse.end += timeoff;
		stse.readorder += GetSize();
		CSTSArray::Add(stse);
	}
...
bool CSimpleTextSubtitle::CopyStyles(const CSTSStyleMap& styles, bool fAppend)
{
	...
	while(pos) 
	{
		CString key;
		void* ptr;
		styles.GetNextAssoc(pos, key, ptr);
...
/*
	...
	for(int i = j-1; i >= 0; i--)
	{
		...
		{
			m_segments.SetSize(m_segments.GetSize()-1);
			j--;
		}
...
void CSimpleTextSubtitle::AddStyle(CString name, STSStyle* style)
{
	...
	void* value;
	if(m_styles.Lookup(name, value)) 
	{
		if(*((STSStyle*)value) == *style)
		{
			delete style;
			return;
		}
		...
		m_styles[name3] = (STSStyle*)value;

		for(i = 0, j = GetSize(); i < j; i++)
		{
			STSEntry& stse = GetAt(i);
...
{
	int i = 0, j = m_segments.GetSize() - 1, ret = -1;
	...
	if(0 <= ret && ret < m_segments.GetSize())
	{
		if(iSegment) *iSegment = ret;
	}

	if(0 <= ret && ret < m_segments.GetSize() 
	&& m_segments[ret].subs.GetSize() > 0
...
{
	return(i < 0 || m_segments.GetSize() <= i ? -1 :
...
{
	qsort(GetData(), GetSize(), sizeof(STSEntry), !fRestoreReadorder ? comp1 : comp2);
	CreateSegments();
}
...
void CSimpleTextSubtitle::CreateSegments()
{
	...
	qsort(breakpoints.GetData(), breakpoints.GetSize(), sizeof(int), intcomp);

	int* ptr = breakpoints.GetData(), prev = ptr ? *ptr : NULL;

	for(i = breakpoints.GetSize(); i > 0; i--, ptr++)
	{
		if(*ptr != prev) 
		{
			STSSegment stss;
	...
	for(i = 0, j = m_segments.GetSize(); i < j; i++)
	{
		...
		for(int k = 0, l = stss.subs.GetSize(); k < l; k++)
		{
...
bool CSimpleTextSubtitle::SaveAs(CString fn, exttype et, double fps, CTextFile::enc e)
{
	...
	else if(et == EXTSSA || et == EXTASS)
	{
		...
		while(pos) 
		{
			CString key;
			void* ptr;
			m_styles.GetNextAssoc(pos, key, ptr);
			STSStyle* s = (STSStyle*)ptr;
...
//	Sort(true);
	...
	for(int i = 0, j = GetSize(), k = 0; i < j; i++)
	{
		STSEntry& stse = GetAt(i);
...
//	Sort();
	...
	void* val;
	if(!m_fUsingAutoGeneratedDefaultStyle && m_styles.Lookup(_T("Default"), val) && et != EXTSSA && et != EXTASS)
	{
		STSStyle* s = (STSStyle*)val;
