...
DWORD CMatroskaSplitterFilter::ThreadProc()
{
	...
	while(!pFile) // not that pFile would ever change... :P
	{
...
//					pPin->DeliverBeginFlush();
				...
				else
				{
					...
					POSITION pos1 = pFile->m_segment.Cues.GetHeadPosition();
					while(pos1)
					{
						Cue* pCue = pFile->m_segment.Cues.GetNext(pos1);
						POSITION pos2 = pCue->CuePoints.GetTailPosition();
						while(pos2)
						{
							CuePoint* pCuePoint = pCue->CuePoints.GetPrev(pos2);

							if(m_rtStart < (REFERENCE_TIME)(pCuePoint->CueTime*pFile->m_segment.SegmentInfo.TimeCodeScale/100)) continue;
							...
							while(pos3)
							{
								...
								if(pBlocks->Find(0xA0))
								{
									do
									{
										...
										while(!fFoundKeyFrame && !fPassedCueTime && pos4)
										{
											Block* b = blocks.GetNext(pos4);
											if((REFERENCE_TIME)((c.TimeCode+b->TimeCode)*pFile->m_segment.SegmentInfo.TimeCodeScale/100) > m_rtStart) 
												fPassedCueTime = true;
									...
									while(!fFoundKeyFrame && !fPassedCueTime && pBlocks->Next(true));
								}
		...
		case CMD_RUN:
			...
			do
			{
				...
				do
				{
					...
					bool fNext = true;
					while(BlockNumber > 1 && (fNext = pBlocks->Next(true))) BlockNumber--;
					if(!fNext) break;
					...
					while(pos && !m_fSeeking && SUCCEEDED(hr) && !CheckRequest(&cmd))
					{
						...
						if(b->BlockDuration == Block::INVALIDDURATION)
						{
							...
							while(pos && !m_fSeeking)
							{
								...
								if(b->TrackNumber == b2->TrackNumber)
								{
			...
			while(pos && !m_fSeeking && SUCCEEDED(hr) && !CheckRequest(&cmd))
			{
				...
				b->BlockDuration.Set((INT64)pFile->m_segment.SegmentInfo.Duration - b->TimeCode);
...
/*
			CAutoPtr<Cluster> c(new Cluster());
			c->Parse(pCluster);

			do
			{
				CAutoPtr<Cluster> cnext;
				if(pCluster->Next(true))
				{
					cnext.Attach(new Cluster());
					cnext->Parse(pCluster);
				}

				TRACE(_T("pCluster->m_start=%I64d, pCluster->m_next=%I64d, c.TimeCode=%I64d\n"), 
					pCluster->m_start,
					pCluster->m_next,
					c->TimeCode);
				...
				POSITION pos = c->Blocks.GetHeadPosition();

				if(BlockNumber) TRACE(_T("BlockNumber=%d\n"), BlockNumber);
				while(BlockNumber-- > 1)
					c->Blocks.GetNext(pos);
				BlockNumber = 0;

				while(pos && !m_fSeeking)
				{
					Block* b = c->Blocks.GetNext(pos);

					CMatroskaSplitterOutputPin* pPin = NULL;
					if(!m_mapTrackToPin.Lookup(b->TrackNumber, pPin) || !pPin 
					|| !pPin->IsConnected() || !pActivePins.Find(pPin))
						continue;

					TrackEntry* pTE = NULL;
					if(!m_mapTrackToTrackEntry.Lookup(b->TrackNumber, pTE) || !pTE)
						continue;
#ifndef READFULLBLOCKS
                    if(FAILED(hr = pFile->SeekTo(b->BlockPos)))
						break;
#endif
					ASSERT(b->BlockLens.GetSize() > 0);

					if(b->BlockLens.GetSize() == 0)
						break;

					REFERENCE_TIME 
						rtNextStart = (c->TimeCode + b->TimeCode)*tcs/100 + 1;

					if(b->BlockDuration > 0)
					{
						rtNextStart = (c->TimeCode + b->TimeCode + b->BlockDuration)*tcs/100;
					}
					else 
					{
						POSITION tmp = pos;
						while(tmp && b->TrackNumber != c->Blocks.GetAt(tmp)->TrackNumber)
							c->Blocks.GetNext(tmp);

						if(tmp)
							rtNextStart = (c->TimeCode + c->Blocks.GetAt(tmp)->TimeCode)*tcs/100;
						else if(cnext) // TODO: make sure the first block of cnext has the same "TrackNumber"
							rtNextStart = (cnext->TimeCode)*tcs/100;
					}

					rtNextStart -= rtSegmentStart;

					REFERENCE_TIME 
						rtStart = (c->TimeCode + b->TimeCode)*tcs/100 - rtSegmentStart, 
						rtDelta = (rtNextStart - rtStart) / b->BlockLens.GetSize(), 
						rtStop = rtStart + rtDelta;

					m_rtCurrent = rtSegmentStart + rtStart;

					BOOL bDiscontinuity = !bDiscontinuitySent.Find(b->TrackNumber);

#ifdef READFULLBLOCKS
					POSITION pos2 = b->BlockData.GetHeadPosition();
					while(pos2 && !m_fSeeking)
					{
						CBinary* pBlockData = b->BlockData.GetNext(pos2);
#else
					POSITION pos2 = b->BlockLens.GetHeadPosition();
					while(pos2 && !m_fSeeking)
					{
						QWORD len = b->BlockLens.GetNext(pos2);
#endif
						CComPtr<IMediaSample> pSample;
						BYTE* pData;
						if(FAILED(hr = pPin->GetDeliveryBuffer(&pSample, NULL, NULL, 0))
						...
						|| FAILED(hr = pPin->Deliver(pSample)))
						{
							if(POSITION pos = pActivePins.Find(pPin))
								pActivePins.RemoveAt(pos);

							if(!pActivePins.IsEmpty()) // only die when all pins are down
								hr = S_OK;
						...
						if(bDiscontinuity)
							bDiscontinuitySent.AddTail(b->TrackNumber);
					}

					if(!pos && !cnext)
					{
						POSITION pos2 = m_pOutputs.GetHeadPosition();
						while(pos2) m_pOutputs.GetNext(pos2)->DeliverEndOfStream();
					}

					if(CheckRequest(&cmd))
						hr = E_FAIL; // not really fail, just using it to break out from here
...
HRESULT CMatroskaSplitterFilter::DeliverBlock(Block* b)
{
	...
	REFERENCE_TIME 
		rtStart = (b->TimeCode)*m_pInput->GetFile()->m_segment.SegmentInfo.TimeCodeScale/100 - m_rtStart, 
		rtStop = (b->TimeCode + b->BlockDuration)*m_pInput->GetFile()->m_segment.SegmentInfo.TimeCodeScale/100 - m_rtStart,
