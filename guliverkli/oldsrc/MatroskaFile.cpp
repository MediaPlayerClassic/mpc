...
{
	CheckPointer(m_pAsyncReader, E_NOINTERFACE);
	HRESULT hr = m_pAsyncReader->SyncRead(m_pos, sizeof(var), (BYTE*)&var);
	if(S_OK == hr) m_pos += sizeof(var);
...
HRESULT Block::Parse(CMatroskaNode* pMN0, bool fFull)
{
	...
	case 0xA1:
	{
		...
		lens.AddTail(pMN->m_next - pMN->GetPos() - tlen);
...
HRESULT CByte::Parse(CMatroskaNode* pMN)
{
	m_val = 0;
	return pMN->Read(m_val);
}

HRESULT CShort::Parse(CMatroskaNode* pMN)
{
	m_val = 0;
	return pMN->Read(m_val);
}
...
{
	m_start = 0;
	m_len.Set(m_next = m_pMF->GetLength()); 
}
...
HRESULT CMatroskaNode::Parse(CMatroskaNode* pMN)
{
	m_id.Parse(pMN);
	m_len.Parse(pMN);
	m_start = pMN->GetPos();
	m_next = m_start + m_len;
...
bool CMatroskaNode::Next(bool fSame)
{
	...
	while(SUCCEEDED(m_pMF->SeekTo(m_next))
	&& m_pMF->GetPos() < m_pParent->m_next)
	{
		Parse(this);
...
bool CMatroskaNode::Find(DWORD id)
{
	...
	if(pos && SUCCEEDED(m_pMF->SeekTo(pos)))
	{
		Parse(this);
	}
	else
