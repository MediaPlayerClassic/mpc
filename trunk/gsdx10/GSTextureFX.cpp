...
bool GSTextureFX::Create(GSDevice* dev)
{
	...
	hr = CompileShader(*m_dev, &m_vs, IDR_TFX_FX, _T("vs_main"), il, countof(il), &m_il);
...
bool GSTextureFX::SetupGS(GSSelector sel)
{
	...
	if(sel.prim > 0 && (sel.iip == 0 || sel.prim == 3)) // geometry shader works in every case, but not needed
	{
		if(!m_gs.Lookup(sel.GetHash(), gs))
		{
			...
			str[0].Format(_T("%d"), sel.iip);
			str[1].Format(_T("%d"), sel.prim);
			...
			hr = CompileShader(*m_dev, &gs, IDR_TFX_FX, _T("gs_main"), macro);
...
void GSTextureFX::UpdatePS(PSSelector sel, PSSamplerSelector ssel)
{
	...
	if(!m_ps.Lookup(sel.GetHash(), ps))
	{
		...
		str[0].Format(_T("%d"), sel.fst);
		str[1].Format(_T("%d"), sel.clamp);
		str[2].Format(_T("%d"), sel.bpp);
		str[3].Format(_T("%d"), sel.aem);
		str[4].Format(_T("%d"), sel.tfx);
		str[5].Format(_T("%d"), sel.tcc);
		str[6].Format(_T("%d"), sel.ate);
		str[7].Format(_T("%d"), sel.atst);
		str[8].Format(_T("%d"), sel.fog);
		str[9].Format(_T("%d"), sel.clr1);
		str[10].Format(_T("%d"), sel.fba);
		str[11].Format(_T("%d"), sel.aout);
		...
		hr = CompileShader(*m_dev, &ps, IDR_TFX_FX, _T("ps_main"), macro);
...
void GSTextureFX::UpdateOM(OMDepthStencilSelector dssel, OMBlendSelector bsel, float bf)
{
	...
	if(!m_om_bs.Lookup(bsel.GetHash(), bs))
	{
		...
		if(bsel.abe)
		{
			...
			D3D10_BLEND_OP op = D3D10_BLEND_OP_ADD;
			D3D10_BLEND src = D3D10_BLEND_SRC1_ALPHA;
			D3D10_BLEND dst = D3D10_BLEND_INV_SRC1_ALPHA;
