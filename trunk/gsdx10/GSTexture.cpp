...
{
	m_tc->m_state->m_perfmon.Put(GSPerfMon::WriteTexture, 1);
	...
	hr = m_tc->m_state->m_dev.CreateTexture(m_texture, w, h, format);
...
bool GSTextureCache::GSTexture::Create(GSRenderTarget* rt)
{
	...
	m_tc->m_state->m_perfmon.Put(GSPerfMon::ConvertRT2T, 1);
	...
	if(rt->m_TEX0.TBW != m_TEX0.TBW) // && rt->m_TEX0.PSM == m_TEX0.PSM
	{
		...
		hr = m_tc->m_state->m_dev.CreateRenderTarget(m_texture, rt->m_texture.m_desc.Width, rt->m_texture.m_desc.Height);
		...
		for(int dy = 0; dy < dh; dy += bh)
		{
			for(int dx = 0; dx < dw; dx += bw)
			{
				...
				m_tc->m_state->m_dev.StretchRect(rt->m_texture, src, m_texture, dst);
	...
	if(w != rt->m_texture.m_desc.Width || h != rt->m_texture.m_desc.Height)
	{
		...
		hr = m_tc->m_state->m_dev.CreateRenderTarget(*dt, w, h);

		if(src == dst)
		{
			D3D10_BOX box = {0, 0, 0, w, h, 1};

			m_tc->m_state->m_dev->CopySubresourceRegion(*dt, 0, 0, 0, 0, *st, 0, &box);
		}
		else
		{
			...
			m_tc->m_state->m_dev.StretchRect(*st, src, *dt, dst);
		}

		if(tmp)
		{
			m_tc->m_state->m_dev.Recycle(m_texture);
	...
	if(!m_texture)
	{
		hr = m_tc->m_state->m_dev.CreateTexture(m_texture, rt->m_texture.m_desc.Width, rt->m_texture.m_desc.Height);

		m_tc->m_state->m_dev->CopyResource(m_texture, rt->m_texture);
	}
	...
	case PSM_PSMT8H:
		m_bpp2 = 4;
		hr = m_tc->m_state->m_dev.CreateTexture(m_palette, 256, 1, m_TEX0.CPSM == PSM_PSMCT32 ? DXGI_FORMAT_R8G8B8A8_UNORM : DXGI_FORMAT_R16_UNORM); // 
		break;
	case PSM_PSMT4HL:
	case PSM_PSMT4HH:
		ASSERT(0); // TODO
...
void GSTextureCache::GSTexture::Update(GSLocalMemory::readTexture rt)
{
	...
	(s->m_mem.*rt)(r, bits, pitch, s->m_context->TEX0, s->m_env.TEXA, s->m_context->CLAMP);
