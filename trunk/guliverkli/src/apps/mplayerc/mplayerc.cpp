	...
	else
	{
		...
		*((DWORD*)&RealMediaQuickTimeFPS) = pApp->GetProfileInt(ResStr(IDS_R_SETTINGS), ResStr(IDS_RS_REALMEDIAFPS), *((DWORD*)&RealMediaQuickTimeFPS));
		...
		if(!pApp->GetProfileInt(_T("Shaders"), _T("Initialized"), 0))
		{
			...
			pApp->WriteProfileString(_T("Shaders"), _T("1"), _T("emboss") + hdr + _T("\\tfloat dx = 1/width;\\n\\tfloat dy = 1/height;\\n\\t\\n\\tfloat4 c1 = tex2D(s0, t0 + float2(-dx,-dy));\\n\\tfloat4 c2 = tex2D(s0, t0 + float2(0,-dy));\\n\\tfloat4 c4 = tex2D(s0, t0 + float2(-dx,0));\\n\\tfloat4 c6 = tex2D(s0, t0 + float2(dx,0));\\n\\tfloat4 c8 = tex2D(s0, t0 + float2(0,dy));\\n\\tfloat4 c9 = tex2D(s0, t0 + float2(dx,dy));\\n\\n\\tfloat4 c0 = (-c1-c2-c4+c6+c8+c9);\\n\\tc0 = (c0.r+c0.g+c0.b)/3 + 0.5;\\n") + ftr);
			pApp->WriteProfileString(_T("Shaders"), _T("2"), _T("invert") + hdr + _T("\\tfloat4 c0 = float4(1, 1, 1, 1) - tex2D(s0, t0);\\n") + ftr);
			pApp->WriteProfileString(_T("Shaders"), _T("3"), _T("letterbox") + hdr + _T("\\tfloat4 c0 = 0;\\n\\t\\n\\tfloat2 ar = float2(16, 9);\\n\\tfloat h = (1 - width/height * ar.y/ar.x) / 2;\\n\\t\\n\\tif(t0.y >= h && t0.y <= 1-h)\\n\\t\\tc0 = tex2D(s0, t0);\\n") + ftr);
			pApp->WriteProfileString(_T("Shaders"), _T("4"), _T("spotlight") + hdr + _T("\\tfloat4 c0 = tex2D(s0, t0);\\n\\tfloat3 lightsrc = float3(sin(clock*3.1415/1500)/2+0.5,cos(clock*3.1415/1000)/2+0.5,1);\\n\\tfloat3 light = normalize(lightsrc - float3(t0.x,t0.y,0));\\n\\tc0 *= pow(dot(light, float3(0,0,1)), 50);\\n") + ftr);
			pApp->WriteProfileString(_T("Shaders"), _T("5"), _T("wave") + hdr + _T("\\t// don't look at this for too long, you'll get dizzy :)\\n\\n\\tfloat4 c0 = 0;\\n\\n\\tt0.x += sin(t0.x+clock/300)/20;\\n\\tt0.y += sin(t0.x+clock/300)/20;\\n\\n\\tif(t0.x >= 0 && t0.x <= 1 && t0.y >= 0 && t0.y <= 1)\\n\\t{\\n\\t\\tc0 = tex2D(s0, t0);\\n\\t}\\n") + ftr);
		}
