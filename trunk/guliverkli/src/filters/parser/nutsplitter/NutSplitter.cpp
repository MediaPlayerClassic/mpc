...
{
	{L"Nut Splitter", &__uuidof(CNutSplitterFilter), CNutSplitterFilter::CreateInstance, NULL, &sudFilter[0]},
	{L"Nut Source", &__uuidof(CNutSourceFilter), CNutSourceFilter::CreateInstance, NULL, &sudFilter[1]},
};
...
CUnknown* WINAPI CNutSplitterFilter::CreateInstance(LPUNKNOWN lpunk, HRESULT* phr)
{
    CUnknown* punk = new CNutSplitterFilter(lpunk, phr);
    if(punk == NULL) *phr = E_OUTOFMEMORY;
	return punk;
}

CUnknown* WINAPI CNutSourceFilter::CreateInstance(LPUNKNOWN lpunk, HRESULT* phr)
{
    CUnknown* punk = new CNutSourceFilter(lpunk, phr);
    if(punk == NULL) *phr = E_OUTOFMEMORY;
	return punk;
}
