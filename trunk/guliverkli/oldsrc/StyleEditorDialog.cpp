...
void CStyleEditorDialog::OnBnClickedApplyNow()
{
	if(!m_pstss) return;
	...
	*m_pstss = m_stss;
	...
	if(m_pParent && m_pParent->IsKindOf(RUNTIME_CLASS(CPlayerSubresyncBar)))
		((CPlayerSubresyncBar*)m_pParent)->SaveSubtitle();
    else if(m_pParent && m_pParent->IsKindOf(RUNTIME_CLASS(CPropertyPage)))
		((CPropertyPage*)m_pParent)->OnApply();
}
