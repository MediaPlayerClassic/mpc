...
__declspec(align(16)) union GSSoftVertexFX
{
	...
	public:
		...
		explicit Scalar(int i) {val = i << 16;}
...
#endif
		void abs() {val = ::abs(val);}

		Scalar floor_s() const {Scalar s; s.val = val & 0xffff0000; return s;}
		int floor_i() const {return val >> 16;}

		Scalar ceil_s() const {Scalar s; s.val = (val + 0xffff) & 0xffff0000; return s;}
		int ceil_i() const {return (val + 0xffff) >> 16;}

		void operator = (float f) {val = (int)(f*65536);}
		void operator = (int i) {val = i << 16;}
		...
		friend Scalar operator + (const Scalar& s1, const Scalar& s2) {Scalar s; s.val = s1.val + s2.val; return s;}
		friend Scalar operator - (const Scalar& s1, const Scalar& s2) {Scalar s; s.val = s1.val - s2.val; return s;}
		friend Scalar operator * (const Scalar& s1, const Scalar& s2) {Scalar s; s.val = fixmul(s1.val, s2.val); return s;}
		friend Scalar operator / (const Scalar& s1, const Scalar& s2) {Scalar s; s.val = fixdiv(s1.val, s2.val); return s;}

		friend Scalar operator + (const Scalar& s1, int i) {Scalar s; s.val = s1.val + i; return s;}
		friend Scalar operator - (const Scalar& s1, int i) {Scalar s; s.val = s1.val - i; return s;}
		friend Scalar operator * (const Scalar& s1, int i) {Scalar s; s.val = s1.val * i; return s;}
		friend Scalar operator / (const Scalar& s1, int i) {Scalar s; s.val = s1.val / i; return s;}

		friend Scalar operator << (const Scalar& s1, int i) {Scalar s; s.val = s.val << i; return s;}
		friend Scalar operator >> (const Scalar& s1, int i) {Scalar s; s.val = s.val >> i; return s;}
	...
	public:
		union
		{
			union {struct {Scalar x, y, z, q;}; struct {Scalar r, g, b, a;}; struct {__int64 xy; __int64 zq;};};
		...
		void operator = (Scalar s) {xyzq = _mm_set1_epi32(s.GetValue());}
	...
	void operator += (const GSSoftVertexFX& v) 
	{
		c += v.c; 
		p.x += v.p.x; 
		p.y += v.p.y; 
	...
	static void Exchange(GSSoftVertexFX* RESTRICT v1, GSSoftVertexFX* RESTRICT v2)
	{
...
__forceinline GSSoftVertexFX::Vector operator + (const GSSoftVertexFX::Vector& v, const GSSoftVertexFX::Scalar s) {return GSSoftVertexFX::Vector(_mm_add_epi32(v, _mm_set1_epi32(s.GetValue())));}
__forceinline GSSoftVertexFX::Vector operator - (const GSSoftVertexFX::Vector& v, const GSSoftVertexFX::Scalar s) {return GSSoftVertexFX::Vector(_mm_sub_epi32(v, _mm_set1_epi32(s.GetValue())));}
__forceinline GSSoftVertexFX::Vector operator * (const GSSoftVertexFX::Vector& v, const GSSoftVertexFX::Scalar s) 
{
	...
	__m128 r2 = _mm_cvtepi32_ps(_mm_set1_epi32(s.GetValue()));
	__m128i r0 = _mm_cvtps_epi32(_mm_mul_ps(r1, r2));
	return GSSoftVertexFX::Vector(r0);
}
__forceinline GSSoftVertexFX::Vector operator / (const GSSoftVertexFX::Vector& v, const GSSoftVertexFX::Scalar s)
...
__forceinline GSSoftVertexFX::Vector operator << (const GSSoftVertexFX::Vector& v, int i) {return GSSoftVertexFX::Vector(v.x << i, v.y << i, v.z << i, v.q << i);}
__forceinline GSSoftVertexFX::Vector operator >> (const GSSoftVertexFX::Vector& v, int i) {return GSSoftVertexFX::Vector(v.x >> i, v.y >> i, v.z >> i, v.q >> i);}
