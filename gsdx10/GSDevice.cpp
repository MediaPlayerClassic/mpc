...
bool GSDevice::Create(HWND hWnd)
{
	...
	hr = CompileShader(m_dev, &m_convert.vs, IDR_CONVERT_FX, _T("vs_main"), il_convert, countof(il_convert), &m_convert.il);

	for(int i = 0; i < countof(m_convert.ps); i++)
	{
		CString main;
		main.Format(_T("ps_main%d"), i);
		hr = CompileShader(m_dev, &m_convert.ps[i], IDR_CONVERT_FX, main);
	}
	...
	hr = CompileShader(m_dev, &m_merge.vs, IDR_MERGE_FX, _T("vs_main"), il_merge, countof(il_merge), &m_merge.il);
	hr = CompileShader(m_dev, &m_merge.ps, IDR_MERGE_FX, _T("ps_main"));
	...
	for(int i = 0; i < countof(m_interlace.ps); i++)
	{
		CString main;
		main.Format(_T("ps_main%d"), i);
		hr = CompileShader(m_dev, &m_interlace.ps[i], IDR_INTERLACE_FX, main);
	}
...
void GSDevice::SaveToFileD32S8X24(ID3D10Texture2D* ds, LPCTSTR fn)
{
	...
	::D3DX10SaveTextureToFile(dst, D3DX10_IFF_BMP, fn);
}
...
void GSDevice::Interlace(GSTexture2D& st, GSTexture2D& dt, int shader, bool linear, int yoffset)
{
	InterlaceCB cb;
